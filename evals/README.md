...
- 러너는 `eval <시각>` 프로젝트를 만들고 그 프로젝트에 귀속된 `E-Bolt`/`E-Lint`/`E-Mira` 팀원을 로컬 D1 에 직접 넣어 씁니다(에이전트는 매니저 고용으로만 생기므로). 끝나면 프로젝트(카드·댓글·기억·팀원 포함)를 지웁니다. 그래서 dev 서버가 로컬 D1(`.wrangler/state`)을 쓸 때만 동작합니다.
