...
## 10. 베타 운영 (2026-09-05 결정)

베타 기간 동안 베타 사용자는 **실제 과금 없이** 크레딧을 충전할 수 있습니다. 본인 Claude API 키 연동도 그대로 가능합니다.

| 항목 | 결정 |
|---|---|
| 충전 방식 | 지금 만든 토스 결제창 흐름을 **테스트 키(`test_ck_`/`test_sk_`)** 로 그대로 운영. 결제창은 열리지만 실제 청구는 없음. 별도 "무료 충전" 버튼은 두지 않음 (결제 UX 검증 겸용) |
| 베타 판정 | `betaBilling()` = 결제 활성 + `TOSS_CLIENT_KEY` 가 `test_` 로 시작. 별도 플래그 없음 — "테스트 키 = 무과금" 사실과 어긋날 수 없게. live 키로 바꾸는 순간 자동 해제 |
| 한도 | 사용자당 **월 5,000 크레딧**(`CREDIT_BETA_MONTHLY_CAP`, 0 이면 무제한). KST 매월 1일 초기화. 승인 완료(done) 결제의 크레딧 합으로 계산 — 환불하면 한도가 돌아옴 |
| 한도 초과 | `POST /api/credits/orders` 가 429 `beta_quota_exceeded`. 화면은 남은 한도를 넘는 충전 단위를 비활성 |
| 모델 | 베타 충전분은 유료(paid) 버킷 → **전 모델 허용** (체험 300 만 있을 때의 haiku·sonnet 제한은 유지) |
| 원장 표시 | 베타 중 승인된 charge 행의 `meta.beta = true`. 내역에 "베타 충전" 표시 |
| 베타 종료 | **남은 베타 크레딧 소멸** — `meta.beta` 인 charge 합에서 사용분을 뺀 잔여를 `adjust` 음수 행으로 정리(스크립트는 종료 시점에 작성). 정식 전환 시 감사 크레딧을 소량 지급. 랜딩·계정·FAQ 에 "환불 불가·소멸" 명시 |
| 안내 위치 | 랜딩: 네브바 '앱 열기'·요금 개인 플랜 CTA·마지막 CTA 에 BETA 배지, 요금 섹션 상단 베타 배너, FAQ 첫 항목, 시작 안내 3단계. 앱: 계정 > 크레딧 카드 상단 BETA 안내 + 이번 달 충전량/한도 바 |
| 운영자 비용 | 베타 크레딧 실행은 운영자 `ANTHROPIC_API_KEY` 로 나감 → 사용자당 월 최대 약 5만원 상당(단가 1.8배 기준 실비 ≈ 2.8만원). 베타 인원 × 이 값이 월 상한 |

운영 반영 체크리스트: `TOSS_CLIENT_KEY`(test) vars + `TOSS_SECRET_KEY`(test) 시크릿 + `ANTHROPIC_API_KEY` 시크릿 등록 → `/api/credits` 응답의 `beta.enabled` 가 true 인지 확인 → 충전 한 번 왕복(잔액·내역 "베타 충전" 표시) → 약관 제5조에 베타 문단 추가(다음 작업).
