...
- (결정) 시간당 실행 횟수 제한은 두지 않음 — 잔액 0 이면 실행 불가(402)로 충분
...
| 단계 | 방식 | 비고 |
|---|---|---|
| 1단계 (출시) | 결제창 일반결제(카드)로 수동 충전 + 잔액 부족 안내(앱 안) | 추가 계약 없음. 계좌이체·이메일 알림 없음(결정) |
...
- **잔액 경고**: 50 크레딧 이하 배너, 0 이면 실행 불가 안내(402 → 계정 화면). 이메일 알림은 두지 않음(결정).
...
- **통신판매업 신고**: 완료 — 2026-경기의왕-0174 (와이즈쿼리, 2018-05-24 신고). lib/legal.ts `mailOrderRegistration` 에 기재, 약관·랜딩 푸터 표시. 신고 내용의 인터넷 도메인(oracsil.com)·취급품목에 orbitcrew.ai 를 추가 변경신고할지 확인 권장.
...
### 다음 단계 (4단계)
운영 반영: `wrangler secret put TOSS_SECRET_KEY` + `wrangler.deploy.json` vars 에 `TOSS_CLIENT_KEY`(테스트 키로 먼저), `ANTHROPIC_API_KEY` 시크릿. 약관(/terms) 크레딧 조항·개인정보처리방침 결제 항목. 토스 라이브 키 전환은 가맹 계약·심사 후. 남은 항목에 대한 결정(2026-09-05): 시간당 실행 제한은 두지 않음 — 잔액이 0 이면 실행 불가(402)로 충분. 잔액 부족은 앱 안(배너·402 안내)에서만 알리고 이메일 알림은 없음. 계좌이체는 제공하지 않고, 추후 카드 등록(월 정기 충전, 토스 빌링 계약)으로 대체. 통신판매업 신고번호 2026-경기의왕-0174 를 lib/legal.ts 에 기재(약관 제14조·랜딩 푸터 표시).
