...
export async function POST(request: Request) {
  ...
  // 연결된 작업 폴더의 파일 목록·내용. 서버(Workers)는 파일시스템을 못 보므로 브라우저가 읽어서 보냅니다.
  ...
  try {
    ...
    return Response.json({
      runId, taskId: task.id, status: nextStatus, output, summary,
      blocked, blockedReason, nextActions: done?.nextActions ?? [],
